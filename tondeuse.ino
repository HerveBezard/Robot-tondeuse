...
const char *OTAName = "Sonde1";  // A name and a password for the OTA service
const char *OTAPassword = "MonMotDePasse";
...
int pwm_tondeuse(int canal_pwm, int broche_pwm, int freq_pwm) {
  ledcAttach(broche_pwm, freq_pwm, 8);  // broche 18, canal 0.
...
void vitesse_lame(int valeur) {
  ...
  // Tracer un cercle au centre de l'écran
  ...
  if (valeur == 0) {
    ...
    // Afficher le contenu de l'écran
    Mydisplay.display();
#endif
...
#endif
#if defined(LCD1306)
  // Tracer un rectangle plein à gauche de l'écran
  Mydisplay.setColor(BLACK);
...
#endif
  Serial.print("distance mesurées  : ");
  Serial.print(long_gau);
  Serial.print("   ");
  Serial.print(long_cent);
  Serial.print("   ");
  Serial.println(long_droi);


#if defined(BT_ENABLE)
  SerialBT.print("distance mesurées  : ");
  SerialBT.print(long_gau);
  SerialBT.print(" / ");
  SerialBT.print(long_cent);
  SerialBT.print(" / ");
  SerialBT.println(long_droi);
#if defined(US_SOL)
  SerialBT.print("distance sol  : ");
  SerialBT.println(long_sol);
#endif
#endif
}
...
*/
  ...
  Serial.print("distance mesurees  : ");
  SerialBT.print(long_gau);
  SerialBT.print(" / ");
  SerialBT.print(long_cent);
  SerialBT.print(" / ");
...
#if defined(BT_ENABLE)
  SerialBT.print("distance mesurees  : ");
  SerialBT.print(long_gau);
  SerialBT.print(" / ");
  SerialBT.print(long_cent);
  SerialBT.print(" / ");
...
*/
  ...
  WiFi.mode(WIFI_STA);
  WiFiMulti.addAP("MONSSID1", "MonMotDePasse1");
#if defined(LCD1306)
...
void loop() {
  ...
  switch (mon_mode) {
    ...
    case MODE_TONDEUSE:  // mode tondeuse
      ...
      statut_choc = check_bump();
      if (statut_choc > 0) {
        //les capteurs de chocs sont activ?
        stop_mot();
#if defined(BT_ENABLE)
        SerialBT.print(" statut choc");
        SerialBT.println(statut_choc);
        ...
        // les capteurs de chocs ne sont pas activ?s
